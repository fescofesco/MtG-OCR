...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            gen_uis = get_roi(card, get_coordinates_from_file('ui'),"ui s",0)
            roi = list(gen_uis)  # Collect all ROIs into a list
        
            gen_name = get_roi(card, get_coordinates_from_file('name'),"name",0)
            rois = list(gen_name)  # Collect all ROIs into a list
            ...
            for roi in rois:
                ...
                if roi_text != "":
                    print("Extracted Text ROIS:")
                    roi_text = roi_text.replace('\n', '')
                    print(roi_text)
