...
Main function is 
    identify_card(path_to_img_folder,direcotry = "ImgStorage", verbose=1):
...
def identify_card(path_to_img, direcotry = "ImgStorage",  verbose=0):
    """
    ...
    if not os.path.isabs(path_to_img):  # Check if the path is relative
        full_path = os.path.join(direcotry, path_to_img)
    else:
    ...
    try:
        ...
        if original_image is None:
            error_message = "Image not loaded. Check the image path or file integrity."
            if verbose >0: print(error_message)
            return None, error_message
        
        else:
            # Proceed with image processing
            # image is the resized small image -> easier card contour identification
      ...
      warning_message = "can't open/read file"
      if warning_message in str(e):
          error_message = "Image file could not be opened or read. Check file path or integrity."
          if verbose > 0:
              print(error_message)
          return None, error_message
      else:
...
if __name__ == "__main__":
    identify_card("test23.jpg","ImgStorage", 1)
    identify_card("7.jpg", verbose = 2)
