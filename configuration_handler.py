...
class MtGOCRData(ConfigHandler):
    def __init__(self, parameters_file="parameters.txt"):
      ...
      ImgStorage directory (Default called ImgStorage) folder
...
General parameters
  ...
  "img_storage_directory": where the card images are saved to, 
                          default "working directory / ImgStorage"
...
The default values are defining two edges of a rectanlge of the card snippet
  ...
  "Mtg_letters": all letters used in MTG card names, to limit OCR to these.
    ...
    def create_default_config(self):
        ...
        default_config = {
            "img_storage_directory": os.path.join(os.getcwd(), "ImgStorage"),
    ...
    def get_img_directory(self):
        ...
        if value is None or value == "":
            self.set_img_storage_directory(os.path.join(os.getcwd(), "ImgStorage"))
        return value

    def set_img_directory(self, value=None, tkinterDialog = True):
        if value is None:
            if tkinterDialog == True:
                ...
                value = self.prompt_for_parameter_value("img_storage_directory")

        if value == "default":
            value = os.path.join(os.getcwd(), "ImgStorage")
